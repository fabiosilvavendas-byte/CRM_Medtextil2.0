...
# MÓDULO: PERFORMANCE COMERCIAL (DIAGNÓSTICO ROBUSTO & PDF)
# ==============================================================================
# CERTIFIQUE-SE QUE O NOME ABAIXO É IGUAL AO QUE ESTÁ NO SEU SELECTBOX DA SIDEBAR
if menu == "Performance de Vendedores": 
    ...
    # --- 1. CAMADA DE PADRONIZAÇÃO (Evita KeyError) ---
    def padronizar_colunas_vendas(df_input):
        mapa = {
            'vendedor': ['vendedor', 'funcionario', 'representante', 'consultor'],
            'valor': ['vlr. total', 'totalproduto', 'valor', 'valor liquido', 'faturamento'],
            'data': ['dataemissao', 'data', 'emissao', 'dt_venda'],
            'produto': ['descricao', 'descrição', 'produto', 'item'],
            'cliente': ['cpf_cnpj', 'cnpj', 'cliente', 'razao social'],
        ...
        for logico, variantes in mapa.items():
            col_original = next((c for c in df_input.columns if c.lower().strip() in variantes), None)
            if col_original:
                df_new = df_new.rename(columns={col_original: logico})
    ...
    # Processando os dados que já foram filtrados pela sua Sidebar
    df_analise = padronizar_colunas_vendas(df_filtrado)

    if not df_analise.empty:
        # --- 2. CÁLCULO DE MÉTRICAS RESILIENTES ---
        # Separar vendas de devoluções se a coluna existir
        if 'movimento' in df_analise.columns:
            vendas = df_analise[df_analise['movimento'].str.contains('Venda', case=False, na=False)]
            devolucao = df_analise[df_analise['movimento'].str.contains('Devolu', case=False, na=False)]
        else:
            vendas = df_analise
            devolucao = pd.DataFrame()

        # KPIs (Usando sua função obter_notas_unicas para Ticket Médio real)
        notas_vendas = obter_notas_unicas(vendas) if 'vendas' in locals() else vendas
        fat_bruto = notas_vendas['valor'].sum() if 'valor' in notas_vendas.columns else 0
        fat_dev = devolucao['valor'].sum() if not devolucao.empty and 'valor' in devolucao.columns else 0
        fat_liq = fat_bruto - fat_dev
        
        n_pedidos = len(notas_vendas)
        n_clientes = vendas['cliente'].nunique() if 'cliente' in vendas.columns else 0
        ticket_medio = fat_liq / n_pedidos if n_pedidos > 0 else 0
        mix_itens = vendas['produto'].nunique() if 'produto' in vendas.columns else 0

        # --- 3. EXIBIÇÃO NO DASHBOARD ---
        st.subheader("📌 Resumo Executivo")
        c1, c2, c3, c4 = st.columns(4)
        with c1: render_kpi_card("Faturamento Líquido", f"R$ {fat_liq:,.2f}", f"Devol: R$ {fat_dev:,.2f}")
        with c2: render_kpi_card("Ticket Médio", f"R$ {ticket_medio:,.2f}", f"Base: {n_pedidos} Ped.")
        with c3: render_kpi_card("Positivação", f"{n_clientes} Clientes", icon="👥")
        with c4: render_kpi_card("Mix de Produtos", f"{mix_itens} Itens", icon="📦")
        ...
        with g1:
            if 'data' in df_analise.columns:
                evol = notas_vendas.groupby(notas_vendas['data'].dt.date)['valor'].sum().reset_index()
                fig_evol = px.area(evol, x='data', y='valor', title="Tendência de Faturamento", color_discrete_sequence=['#1F4788'])
                st.plotly_chart(aplicar_layout_grafico(fig_evol), use_container_width=True)
        
        with g2:
            if 'produto' in df_analise.columns:
                rank_prod = vendas.groupby('produto')['valor'].sum().nlargest(10).reset_index()
                fig_prod = px.bar(rank_prod, x='valor', y='produto', orientation='h', title="Top 10 Produtos (Valor)")
                st.plotly_chart(aplicar_layout_grafico(fig_prod), use_container_width=True)

        # --- 4. EXPORTAÇÃO PDF SOB DEMANDA ---
        def preparar_pdf():
            pdf = FPDF()
            pdf.add_page()
            pdf.set_fill_color(31, 71, 136) # Cor MedTextil
            pdf.rect(0, 0, 210, 40, 'F')
            pdf.set_text_color(255, 255, 255)
            pdf.set_font("Arial", 'B', 16)
            pdf.cell(190, 20, "RELATÓRIO DE PERFORMANCE COMERCIAL", 0, 1, 'C')
            
            pdf.set_text_color(0, 0, 0)
            pdf.ln(25)
            pdf.set_font("Arial", 'B', 12)
            pdf.cell(190, 10, "DIAGNÓSTICO EXECUTIVO", 0, 1, 'L')
            ...
            metricas_pdf = [
                ...
                ["Mix de Produtos", str(mix_itens)],
                ["Inadimplência (Valor)", "Consulte painel"] # Exemplo de métrica condicional
            ]
            for m_label, m_valor in metricas_pdf:
                pdf.cell(100, 10, m_label, 1)
                pdf.cell(90, 10, m_valor, 1, 1, 'C')

            return pdf.output(dest='S').encode('latin-1', 'replace')

        st.markdown("---")
        if st.button("📄 Gerar Relatório PDF"):
            pdf_data = preparar_pdf()
            st.download_button("📥 Baixar Relatório", pdf_data, "Performance_Comercial.pdf", "application/pdf")

    else:
        st.info("💡 Por favor, selecione os filtros na barra lateral para carregar o relatório.")        
# ====================== CONSULTA CLIENTES ======================
